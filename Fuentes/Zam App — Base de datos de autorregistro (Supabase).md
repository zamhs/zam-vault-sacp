...
relacionado_con: ["[[MOC - Fuentes]]", "[[Protocolo de Ingesta de Datos Cuantitativos]]", "[[Protocolo de Asignación de Tareas Personalizadas]]", "[[Arquitectura Cognitiva del Sistema]]"]
autor: Zam
...
**Regla de acceso: solo lectura, con una excepción estructural.** Desde este sistema se usa por defecto `execute_sql` con consultas `SELECT`. Nunca se ejecuta `INSERT`/`UPDATE`/`DELETE`/`apply_migration` sobre este proyecto salvo pedido explícito de Zam en la conversación — con dos excepciones documentadas: la corrección de seguridad de RLS descrita en `Historial de actualizaciones` (aplicada una sola vez), y el [[Protocolo de Asignación de Tareas Personalizadas]] (2026-08-06), que sí implica escritura recurrente sobre `dev_tasks`/`dev_notes`, tanto desde Claude Code como desde Claude.ai, siempre bajo pedido explícito de Zam en cada sesión.
...
- `dev_tasks` — tareas con `priority` (alta/media/baja), `due_date` y `goal_id` (vínculo a `dev_goals`), más tiempo límite, `estimated_minutes` y `quality` (escala 1-10, agregada 2026-08-06, mismo patrón que `dev_daily_logs.quality`, NULL hasta completarse). Soporta dos modos de captura: registrar una tarea ya hecha (retroactivo) o planificarla (`completed:false`, sin fecha de finalización) — permite medir procrastinación real (brecha `due_date` vs `completed_at`), no solo trabajo ya terminado. Desde 2026-08-06, `completed:false` también recibe inserciones/actualizaciones desde chats de IA (Claude Code o Claude.ai) bajo el [[Protocolo de Asignación de Tareas Personalizadas]], visibles en la app en la sección "Tareas asignadas" de `/tareas`. **Ver nota de fecha de corte abajo: antes del 2026-07-20 `priority`/`due_date`/`goal_id` no eran señal real.**
...
- 2026-08-06: se implementó el [[Protocolo de Asignación de Tareas Personalizadas]], a pedido de Zam tras replantear un diseño inicial de generación automática diaria — prefirió poder pedir tareas personalizadas bajo demanda en cualquier chat con conector de Supabase (Claude Code o Claude.ai), en vez de una tarea programada. Migración `add_quality_to_tasks`: nueva columna `dev_tasks.quality` (smallint 1-10) + `COMMENT ON TABLE/COLUMN` en `dev_tasks`/`dev_notes` como red de seguridad para chats sin acceso a este vault. Nueva sección "Tareas asignadas" en `/tareas` (`mis_estadisticas/web/app/tareas/page.tsx`), entre "To-Do" y "Ya la hice", que lista `dev_tasks` con `completed=false` y permite completarlas o descartarlas desde la UI. Verificado de punta a punta con una tarea de prueba real sobre la actividad "Inglés": asignación, consulta de continuidad (`dev_notes`/`dev_tasks` previas), completado desde la UI (con upsert correcto de `dev_daily_logs`) y descarte — datos de prueba limpiados al terminar.
